...
class GradientDescentSolver(Solver):
    ...
    def get_updates_without_momentum(self, loss_tensor, param_vars):
        ...
        updates = []
        "*** YOUR CODE HERE ***"
        scaled_gradients = [self.learning_rate * grad for grad in grad_tensors]
        for i in range(len(param_vars)):
            updates.append((param_vars[i], param_vars[i] - scaled_gradients[i]))
        return updates
    ...
    def get_updates_with_momentum(self, loss_tensor, param_vars):
        ...
        for i in range(len(param_vars)):
            ...
            updates.append((param_vars[i], param_vars[i] + new_vel))
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        for iter_ in range(self.iterations):
            ...
            #print session.run(update_ops)
            #session.run(update_ops)
            #val_loss = session.run
...
class StochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        for iter_ in range(self.iterations):
            ...
            train_loss = session.run(loss_tensor, feed_dict={placeholders[0]: train_batch[0], placeholders[1]: train_batch[1]})
            session.run(update_ops, feed_dict={placeholders[0]: train_batch[0], placeholders[1]: train_batch[1]})
            val_loss = session.run(loss_tensor, feed_dict={placeholders[0]: val_batch[0], placeholders[1]: val_batch[1]})
