...
class PerceptronClassifier(object):
    ...
    def train(self, input_train_data, label_train_data, input_val_data, label_val_data, iterations, callback=None):
        ...
        for iteration in range(iterations):
            ...
            for i in range(len(input_train_data)):
                ...
                predictionLabel = self.classify(input_train_data[i])
                #update weights if guess wrong
                if predictionLabel != label_train_data[i]:
                	self.weights[predictionLabel] = self.weights[predictionLabel] - input_train_data[i]
    ...
    def find_high_weight_features(self, label):
        ...
        #sortedKeys return a list of keys based on their values (here weights)
        sortedFeatures = featureWeights.sortedKeys()
        best100Features = sortedFeatures[:100]
