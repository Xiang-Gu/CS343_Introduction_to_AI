...
import solvers
import util
from solvers import MinibatchStochasticGradientDescentSolver
...
def search_hyperparams(train_data, train_labels, val_data, val_labels,
    ...
    for model, (learning_rate, momentum, batch_size) in zip(models, hyperparams):
        "*** YOUR CODE HERE ***"
        solver = MinibatchStochasticGradientDescentSolver(learning_rate, iterations, batch_size, momentum)
        solver.solve(train_data, train_labels, val_data, val_labels, model)
        currentAccuracy = model.accuracy(val_data, val_labels)
        val_accuracies.append(currentAccuracy)
    #find the index of the highest accuracy, which is also the index of the best model and best hyperparams
    index = val_accuracies.index(max(val_accuracies))
