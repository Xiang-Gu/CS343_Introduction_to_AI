...
def dfs(pixels, row, col):
    assert(pixels[col][row] == 0)
    fringe = []
    fringe.append((row, col))
    while len(fringe) > 0:
        row, col = fringe.pop()
        # continue if not 0
        if pixels[col][row] != 0: continue
        # mark it as 0
        pixels[col][row] = 3
        # append adjacent (four) 0 pixels to fringe
        # (row - 1, col + 0), (row + 1, col + 0), (row + 0, col - 1), (row + 0, col + 1)
        for drow, dcol in zip([-1, +1, 0, 0], [0, 0, -1, +1]):
            newrow, newcol = row + drow, col + dcol
            if 0 <= newrow < DIGIT_DATUM_HEIGHT and 0 <= newcol < DIGIT_DATUM_WIDTH and pixels[newcol][newrow] == 0:
                fringe.append((newrow, newcol))
...
def enhancedFeatureExtractorDigit(datum):
    ...
    "*** YOUR CODE HERE ***"
    # add feature num_of_loops which takes three values 0, 1, 2
    # zero loop -> 0, one loop -> 1, two or more loops -> 2
    pixels = copy.deepcopy(datum.getPixels())
    numLoops = -1
    for row in range(DIGIT_DATUM_HEIGHT):
        for col in range(DIGIT_DATUM_WIDTH):
            if pixels[col][row] == 0:
                numLoops += 1
                dfs(pixels, row, col)
    
    if numLoops == 0:
        features["numLoops0"] = 1
        features["numLoops1"] = 0
        features["numLoops2"] = 0
    elif numLoops == 1:
        features["numLoops0"] = 0
        features["numLoops1"] = 1
        features["numLoops2"] = 0
    else:
        features["numLoops0"] = 0
        features["numLoops1"] = 0
        features["numLoops2"] = 1
...
def enhancedPacmanFeatures(state, action):
    ...
    # add new feature "distToNearestGhost",
    ...
    features["distToNearestGhost"] = 1 / (min(ghostDistances) + 1)

    # add new feature "distToNearestFood"
    ...
    features["distToNearestFood"] = 1 / min(foodDistances) if len(foodDistances) > 0 else 0
