...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # compute a distribution of the particles and their weights
        distribution = util.Counter()
        for i in range(self.numParticles):
            distribution[self.particles[i]] += weights[i]

        # restart if all particles have zero weight
        totalWeight = distribution.totalCount()
        if totalWeight == 0:
            self.initializeUniformly(gameState)
        else:
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        "*** YOUR CODE HERE ***"
        self.particles = []
        combinations = self.legalPositions
        ghosts = self.numGhosts
        while ghosts > 1:
            combinations = list(itertools.product(combinations, self.legalPositions))
            ghosts -= 1
    ...
    def observeState(self, gameState):
        ...
        for i in range(self.numParticles):
            ...
            for ghostIndex in range(self.numGhosts):
                if noisyDistances[ghostIndex] is not None:
                    distance = util.manhattanDistance(
                            pacmanPosition, self.particles[i][ghostIndex])
